...
#Tela de exclusão de colaborador
...
def pesquisar():
    ...
    if var == 'Nome':
        ...
        try:
            ...
            def deletar():
                vardelet = vcolab.get()
                querydelet = f"DELETE FROM colaboradores WHERE NOME_COLAB =" \
                             f"'{vardelet}';"
        ...
        try:
            ...
            def deletar():
                ...
                excluir = messagebox.askyesno("Atenção!", "Deseja confirma a "
                                                          ...
                                                          f"de {vardelet}?")
                if excluir:
                    querydelet = f"DELETE FROM colaboradores WHERE USER_COLAB =" \
                                 f"'{vardelet}';"
                    database.dml(querydelet)
                    print("Exclusão realizada.")
...
Label(app, text=f'Exclusão de Colaborador', background="light gray",
...
bl_filtro = Label(app, text="Pesquisar colaborador por:")
