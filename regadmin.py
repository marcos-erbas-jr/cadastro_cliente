...
#Tela de registro de Administradores
...
def gravarDados():
    if cnome.get() != "":
        ...
        vquery = (F"INSERT INTO admins(NOME_ADMIN,CARGO_ADMIN,SETOR_ADMIN, USER_ADMIN, SENHA_ADMIN) VALUES('{vnome}','{vcargo}','{vsetor}','{vuser}','{vsen}');")
        database.dml(vquery)
        print("Registrado como Administrador")


        #Administradores também são colaboradores e devem poder acessar as
        # duas abas, logo, ao cadastrar um administrador também estará
        # cadastrando a pessoa como colaborador
        vquery2 = (F"INSERT INTO colaboradores(NOME_COLAB,CARGO_COLAB,SETOR_COLAB, USER_COLAB, SENHA_COLAB) VALUES('{vnome}','{vcargo}','{vsetor}','{vuser}','{vsen}');")

        database.dml(vquery2)
        print("Registrado como Colaborador")
...
app.title("Registro de Administradores")
