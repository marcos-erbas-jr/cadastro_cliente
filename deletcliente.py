...
def pesquisar():
    ...
    varquery = search.get()
    if var == 'Nome':
        query = f"SELECT * FROM clientes WHERE NOME_CLIENTE LIKE '" \
                f"%{varquery}%'"
        ...
        try:
            ...
            def deletar():
                ...
                excluir = messagebox.askyesno("Atenção!", "Deseja confirma a "
                                                ...
                                                f"de {vardelet}?")
                if excluir:
                    querydelet = f"DELETE FROM clientes WHERE NOME_CLIENTE =" \
                                 f"'{vardelet}';"
        ...
        query = f"SELECT * FROM clientes WHERE EMAIL_CLIENTE LIKE " \
                f"'%{varquery}%'"
        res = database.dql(query)
        print(res)
        resultado = ''
        try:
            ...
            def deletar():
                ...
                excluir = messagebox.askyesno("Atenção!", "Deseja confirma a "
                                                          ...
                                                          f"de {vardelet}?")
                if excluir:
                    querydelet = f"DELETE FROM clientes WHERE EMAIL_CLIENTE =" \
                                 f"'{vardelet}';"
                    database.dml(querydelet)
                    print("Exclusão realizada.")
...
Label(app, text=f'Exclusão de Cliente', background="light gray",
                    foreground="#000",font=("Helvetica",10, "bold")).place(
    x=130, y=-40, width=160,height=120)

##Parte do Filtro

listaFiltro = ["Nome","E-mail"]
