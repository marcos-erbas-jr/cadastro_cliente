...
#Tela de pesquisa de administradores
...
def pesquisar():
    ...
    if var == 'Nome':
        ...
        query = f"SELECT * FROM admins WHERE CARGO_ADMIN LIKE '%{varquery}%'"
        res = database.dql(query)
        resultado = ''
        try:
            x = res[0]
        ...
        query = f"SELECT * FROM admins WHERE SETOR_ADMIN LIKE" \
                f" '%{varquery}%'"
...
vfiltro = StringVar()
bl_filtro = Label(app, text="Pesquisar administrador por:", anchor="w")
bl_filtro.place(x=128, y=10, width=240, height=30)
